...
class PushSenderSubscriber implements EventSubscriberInterface
{
    ...
    public static function getSubscribedEvents()
    {
        return [
            ...
            Event\UserPetitionEvents::PETITION_SHARE => ['sendSharedPetitionPush', -200],
        ];
    }
    ...
    public function sendPushPublishQuestion(Event\Poll\QuestionEvent $event)
    {
        ...
        if ($entity instanceof Question\LeaderNews) {
            $params = [
                $entity->getId(),
                "{$entity->getOwner()->getOfficialTitle()} Discussion",
                $this->preview($entity->getSubject()),
            ];
        } elseif ($entity instanceof Question\Petition) {
            $params = [
                $entity->getId(),
                "{$entity->getOwner()->getOfficialTitle()} Petition",
                $entity->getPetitionTitle(),
            ];
        } elseif ($entity instanceof Question\PaymentRequest) {
            $params = [
                $entity->getId(),
                "{$entity->getOwner()->getOfficialTitle()} Fundraiser",
                $entity->getTitle(),
            ];
        } elseif ($entity instanceof Question\LeaderEvent) {
            $params = [
                $entity->getId(),
                "{$entity->getOwner()->getOfficialTitle()} Event",
                $entity->getTitle(),
            ];
        } else {
            $params = [
                $entity->getId(),
                "{$entity->getOwner()->getOfficialTitle()} Poll",
                $this->preview($entity->getSubject()),
            ];
        }
    ...
    public function sendSharedPetitionPush(Event\UserPetitionShareEvent $event)
    {
        $petition = $event->getPetition();
        $sharer = $event->getSharer();
        $this->pushTask->addToQueue(
            'sendSharedPetitionPush',
            [$petition->getId(), $sharer->getId()]
        );
    }
